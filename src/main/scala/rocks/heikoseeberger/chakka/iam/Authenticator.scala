...
object Authenticator extends Logging {
  ...
  def apply(config: Config)
           (implicit mat: Materializer, readJournal: EventsByPersistenceIdQuery): Behavior[Command] =
  ...
  def apply(lastSeqNo: Long, credentials: Map[String, String], askTimeout: FiniteDuration)
           (implicit mat: Materializer, scheduler: Scheduler, readJournal: EventsByPersistenceIdQuery): Behavior[Command] = {
    Behaviors.receive {
  ...
  private def runProjection(lastSeqNo: Long, authenticator: ActorRef[Authenticator.Command], askTimeout: FiniteDuration)
                           (implicit mat: Materializer, scheduler: Scheduler, readJournal: EventsByPersistenceIdQuery): Unit = {
    implicit val timeout: FiniteDuration = askTimeout
